...
class EmailService {
  constructor() {
    this.fromAddress = process.env.EMAIL_USER || 'noreply@rivaproperties.com';
  }
  ...
  async sendWelcomeEmail(userEmail, userName) {
    const subject = 'Welcome to Riva Properties - Your Account is Ready!';
  ...
  async sendEmailVerification(userEmail, userName, verificationUrl) {
    const subject = 'Verify Your Riva Properties Email Address';
  ...
  async sendPasswordResetEmail(userEmail, resetUrl) {
    const subject = 'Reset Your Riva Properties Password';
  ...
  async sendNewsletterWelcome(userEmail) {
    const subject = 'Welcome to Riva Properties Newsletter 📧';
  ...
  async sendAppointmentNotificationToAdmin(appointment, userDetails) {
    ...
    const adminEmail = process.env.ADMIN_EMAIL || 'admin@rivaproperties.com';
  ...
  async sendUserSuspended(userEmail, userName, days, reason, suspendedUntil) {
    const subject = `Riva Properties Account Suspended - ${days} Day${days > 1 ? 's' : ''}`;
  ...
  async sendUserBanned(userEmail, userName, reason) {
    const subject = 'Riva Properties Account Permanently Banned';
  ...
  async sendUserReactivated(userEmail, userName) {
    const subject = 'Riva Properties Account Reactivated - Welcome Back!';
