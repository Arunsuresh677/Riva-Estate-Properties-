...
const PropertiesHeader: React.FC<PropertiesHeaderProps> = ({
  ...
  return (
    <div className="border-b border-[#E6E0DA] bg-white sticky top-16 md:top-20 z-10">
      <div className="px-4 sm:px-6 md:px-8 py-3 md:py-5">
        <div className="flex items-center justify-between gap-3">
          {/* Left */}
          <div>
            <h1 className="font-syne text-xl md:text-3xl text-[#221410] leading-tight">All Properties</h1>
          ...
          <div className="flex items-center gap-2 md:gap-4 shrink-0">
            ...
            <div className="flex items-center gap-1 md:gap-2">
              ...
              <select
                ...
                className="bg-white border border-[#E6E0DA] rounded-lg px-2 md:px-4 py-1.5 md:py-2 font-manrope text-xs md:text-sm text-[#221410] cursor-pointer focus:outline-none focus:border-[#D4755B] appearance-none pr-6 md:pr-8 bg-no-repeat"
                style={{
            ...
            {/* View toggle */}
            <div className="flex items-center gap-0.5 bg-[#F8F6F6] rounded-lg p-0.5 md:p-1">
              {(['grid', 'list'] as const).map(mode => (
                <button
                  key={mode}
                  onClick={() => handleViewChange(mode)}
                  className={`p-1.5 md:p-2 rounded transition-all ${
                    viewMode === mode ? 'bg-white text-[#D4755B] shadow-sm' : 'text-[#6B7280] hover:text-[#D4755B]'
                  }`}
                >
